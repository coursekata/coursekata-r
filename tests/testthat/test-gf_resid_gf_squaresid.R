...
# gf_square_resid() and gf_squaresid() are generated through the shared
# CourseKata adapter. These are their half of the contract: what the distinct
# generated functions have to keep doing, and what their released signatures,
# caller environments and literal names could otherwise lose in silence.
...
test_that("the alias is a distinct name-aware function from the shared recipe", {
  ...
  expect_false(identical(gf_squaresid, gf_square_resid))
  expect_false(identical(alias, square))
