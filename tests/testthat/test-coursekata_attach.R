test_that("all course packages are listed with version and whether attached", {
  ...
  detachable <- detachable[order(match(detachable, coursekata_pkgs))]

  # detach/re-attach only installed packages
  installed <- pkg_is_installed(detachable)
  names(installed) <- detachable
  detacher(detachable[installed])
  withr::defer(attacher(detachable[installed]))
