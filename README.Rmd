...
- By default, the package will show all startup messages from the dependent packages. To quiet these (like in the output above), you can set `options(coursekata.quiet = TRUE)`
- By default, a few checks are performed on startup to check that the packages are installed, up-to-date, and loading from the correct locations. This can take a couple of seconds; to disable this in environments where time is critical set `options(coursekata.quickstart = TRUE)`
